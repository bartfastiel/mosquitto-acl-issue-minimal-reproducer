...
echo "## this fails silently, with QoS1"
docker run --rm --network=host eclipse-mosquitto:2.0.20 mosquitto_pub -t forbidden -m hi --debug -u joe --pw pwdjoe1 -q1 && echo exit code $?

echo "## this fails silently, with QoS2"
docker run --rm --network=host eclipse-mosquitto:2.0.20 mosquitto_pub -t forbidden -m hi --debug -u joe --pw pwdjoe1 -q2 && echo exit code $?
